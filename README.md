...
- 🧩 **自动标签** - 基于文件名、类型与可用媒体元数据自动分词，标签按用户文件实例隔离
- 📚 **热部署词典** - 支持个人词典和管理员全局规则草稿、预览、发布、回滚及可恢复历史重建
- 🔍 **搜索功能** - 文件名与标签联合搜索，支持多标签全部/任一匹配和当前目录范围
- 🔗 **限时分享链接** - 生成带有效期的文件分享链接，支持密码保护
- 👁️ **文件预览** - 支持图片、视频在线预览
- 🖼️ **自动缩略图** - 为图片和视频自动生成预览缩略图
- 🌐 **公开文件广场** - 用户可以将文件设为公开，供其他用户浏览

自动标签使用纯 Go 分词实现，不影响 `CGO_ENABLED=0` 构建。图片元数据由内置 Provider 提取；安装 `ffprobe` 后可额外提取音视频时长、分辨率、编码和容器信息，未安装时上传与基础标签功能仍可正常使用。自动标签会随公开文件展示，手工标签默认私有，只有明确设置为公开时才会进入文件广场和公开搜索。
