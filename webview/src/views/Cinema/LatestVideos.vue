<template>
  <div v-loading="loading && videos.length === 0" class="cinema-latest">
    <div class="cinema-latest__heading">
      <button type="button" @click="router.back()">
        <el-icon><ArrowLeft /></el-icon>
      </button>
      <h1>最新视频</h1>
    </div>
    <div class="cinema-latest__grid">
      <CinemaVideoCard
    ...
    <el-empty v-if="!loading && videos.length === 0" description="当前影视库没有可播放视频" />
...
<script setup lang="ts">
  ...
  import { getCinemaLatest, type CinemaVideo } from '@/api/cinema'
  ...
  const loadMore = async () => {
    ...
    loading.value = true
    try {
      const response = await getCinemaLatest(requestedRootId, requestedPage, 24)
...
<style scoped>
  ...
  .cinema-latest__heading button:hover {
    border-color: #cfe6ff;
