  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button v-if="!shareResult" type="primary" :loading="sharing" @click="handleConfirmShare">
          {{ t('share.createShare') }}
        </el-button>
        <el-button v-else type="primary" @click="handleCreateAnother">
          {{ t('share.createShare') }}
        </el-button>
      </div>
    </template>
  ...
  const handleConfirmShare = async () => {
    ...
    try {
      ...
      if (res.code === 200) {
        ...
        const expireText =
          shareForm.expire_days === 0 ? t('share.permanent') : t('share.expireDays', { days: shareForm.expire_days })
