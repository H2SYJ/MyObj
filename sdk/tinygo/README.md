...
- `FilesQuery`：按保存目录内的根相对目录、精确或包含名称、MIME、时间等条件分页查询文件元数据，需要 `files.read_metadata`；宿主不会返回订阅保存目录之外的文件。
- `DownloadableItem`：返回 HTTP/HLS URL、稳定 ID、保存目录下的根相对 `save_path`、缩略图和离线下载自定义头。
...
- `FileGet` 和 `FilesQuery` 不限制调用次数，累计最多返回 500 条。
