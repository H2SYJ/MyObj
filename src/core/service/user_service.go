...
func NewUserService(factory *impl.RepositoryFactory, cacheLocal cache.Cache) *UserService {
	return &UserService{
		...
		sessions:   auth.NewSessionStore(cacheLocal),
	}
}
...
func (u *UserService) Login(username, password, challenge string) (*models.JsonResponse, error) {
	...
	if err := u.sessions.Create(uid, user.ID, jwt, ttlSeconds); err != nil {
		logger.LOG.Error("保存登录会话失败", "error", err)
		return nil, err
	}
	res.Token = uid
	res.Power = nil
...
func (u *UserService) Register(req *request.UserRegisterRequest) (*models.JsonResponse, error) {
	...
	if isFirstUse {
		// 首次使用，强制设置为管理员组（ID=1）
		groupID = models.DefaultAdminGroupID
	} else {
		// 非首次使用，获取默认组
		...
		if groupID == models.DefaultAdminGroupID {
...
func (u *UserService) UpdatePassword(req *request.UserUpdatePasswordRequest) (*models.JsonResponse, error) {
	...
	if err := u.sessions.RevokeUser(user.ID); err != nil {
		return nil, fmt.Errorf("密码已更新，但撤销旧登录会话失败: %w", err)
	}
