...
type FileInfo struct {
	ID              string               `gorm:"type:VARCHAR(64);not null;primaryKey;unique" json:"id"`                             // 文件ID，主键且唯一
	Name            string               `gorm:"type:VARCHAR(255);not null;index:file_info_index_1" json:"name"`                    // 文件原名
	RandomName      string               `gorm:"type:VARCHAR(255);not null" json:"random_name"`                                     // 文件存储名（随机生成）
	Size            int                  `gorm:"type:BIGINT;not null" json:"size"`                                                  // 文件大小
	Mime            string               `gorm:"type:VARCHAR(255);not null;index:file_info_index_0" json:"mime"`                    // 文件MIME类型
	ThumbnailImg    string               `gorm:"type:TEXT" json:"thumbnail_img"`                                                    // 缩略图路径
	Path            string               `gorm:"type:TEXT" json:"path"`                                                             // 文件实际存储路径
	FileHash        string               `gorm:"type:TEXT;not null;index:file_info_file_hash_index,length:255" json:"file_hash"`    // 文件哈希值（全量hash）
	FileEncHash     string               `gorm:"type:TEXT" json:"file_enc_hash"`                                                    // 加密文件哈希值
	ChunkSignature  string               `gorm:"type:TEXT;index:file_info_chunk_signature_index,length:255" json:"chunk_signature"` // 分片签名（快速预检）
