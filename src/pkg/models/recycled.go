...
type Recycled struct {
	...
	// 用户ID
	UserID string `gorm:"type:VARCHAR;not null;index:idx_recycled_user_type_created,priority:1" json:"user_id"`
	// 删除时间
...
type RecycledDirectoryNode struct {
	...
	RecycledID       string `gorm:"type:VARCHAR;not null;index;uniqueIndex:idx_recycled_original_dir" json:"recycled_id"`
...
type RecycledDirectoryFile struct {
	ID            uint   `gorm:"primaryKey" json:"id"`
	RecycledID    string `gorm:"type:VARCHAR;not null;index;uniqueIndex:idx_recycled_directory_file" json:"recycled_id"`
	FileID        string `gorm:"type:VARCHAR;not null;uniqueIndex:idx_recycled_directory_file" json:"file_id"`
